...
interface CartItem {
  ...
  quantity: number; products: Product; selected?: boolean;
}
...
function EmptyCartState() {
  return (
    <div className="page-container state-screen">
      <div className="state-icon"><ShoppingCart size={34} /></div>
      <h2 className="state-title">Keranjang Kosong</h2>
      <p className="state-text">Yuk, mulai belanja dan temukan produk favoritmu!</p>
      <Link href="/"><button className="btn btn-primary"><ShoppingBag size={15} /> Mulai Belanja</button></Link>
    </div>
  );
}
...
export default function CartPage() {
  ...
  async function loadCart() {
    ...
    try {
      ...
      setCartItems((data.data || []).map((item: CartItem) => ({ ...item, selected: true })));
    } catch { console.error("Error loading cart"); }
  ...
  async function updateQuantity(cartId: number, newQty: number) {
    ...
    } catch { showToast("Gagal mengupdate keranjang", "error"); }
  }
  ...
  const handleCheckout = () => {
    if (selectedItems.length === 0) { showToast("Pilih produk yang ingin di-checkout", "error"); return; }
  ...
  const subtotal      = selectedItems.reduce((s, i) => s + i.products.price * i.quantity, 0);
      ...
      <header className="page-header-bar">
        <div className="page-header-bar-inner">
          ...
          <div className="header-text">
            ...
            <p className="header-subtitle">{cartItems.length} produk di keranjang kamu</p>
          </div>
        </div>
      </header>

      {/* ── Main ── */}
      <main className="page-main" style={{ maxWidth: 960, flexDirection: "row", alignItems: "flex-start", flexWrap: "wrap" }}>

        {/* Items */}
        <div className="cart-items" style={{ flex: "1 1 480px" }}>
          <div className="cart-header">
            <label className="cart-select-all">
              <input type="checkbox" className="checkbox-custom" checked={allSelected} onChange={toggleSelectAll} />
          ...
          {cartItems.map((item) => (
            <div key={item.id} className="cart-item">
              <div className="cart-item-checkbox">
                <input type="checkbox" className="checkbox-custom" checked={item.selected ?? false} onChange={() => toggleSelect(item.id)} />
              </div>
              <img
              ...
              <div className="cart-item-actions">
                <button className="cart-item-delete" onClick={() => deleteItem(item.id)} aria-label="Hapus">
                  <Trash2 size={17} />
                </button>
                <div className="qty-control">
                  <button className="qty-btn" onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1} aria-label="Kurangi"><Minus size={13} /></button>
                  <input type="text" className="qty-input" value={item.quantity} readOnly />
                  <button className="qty-btn" onClick={() => updateQuantity(item.id, item.quantity + 1)} aria-label="Tambah"><Plus size={13} /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="cart-summary" style={{ flex: "0 0 300px" }}>
          ...
          <button className="btn btn-primary btn-full" style={{ marginTop: 18, padding: "13px" }} onClick={handleCheckout} disabled={selectedItems.length === 0}>
            <CheckCheck size={15} /> Checkout ({selectedItems.length})
          </button>
          <Link href="/" style={{ display: "block", marginTop: 8 }}>
            <button className="btn btn-secondary btn-full">Lanjut Belanja</button>
          </Link>
        </div>
      </main>
